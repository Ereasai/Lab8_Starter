describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const productItem = await page.$('product-item'); // runs document.querySelector
    
    // Grab the shadowRoot of that element (it's a property), then query a button from that shadowRoot.
    
    const shadow = await productItem.getProperty('shadowRoot');
    const addButton = await shadow.$('button');
    
    // Once you have the button, you can click it and check the innerText property of the button.

    await addButton.click();
    const innerText = await addButton.getProperty('innerText');

    // Once you have the innerText property, use innerText.jsonValue() to get the text value of it

    const planeInnerText = await innerText.jsonValue();
    expect(planeInnerText).toBe('Remove from Cart');

    await addButton.click(); // un-click this button.
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // check if each button says "Remove from Cart"
    for (let i = 0; i < prodItems.length; i++) {
      const shadow = await prodItems[i].getProperty('shadowRoot');
      const addButton = await shadow.$('button');
      const innerText = await addButton.getProperty('innerText');
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    // check if each product-item says "Add to Cart"
    for (let i = 0; i < prodItems.length; i++) {
      const shadow = await prodItems[i].getProperty('shadowRoot');
      const addButton = await shadow.$('button');
      const innerText = await addButton.getProperty('innerText');
      const innerTextRaw = await innerText.jsonValue();
      expect(innerTextRaw).toBe("Add to Cart");
    }
